...
public class MainPanel extends JPanel implements ActionListener {
	...
	public MainPanel() {
		...
		JLabel version = new JLabel("  Versión 1.0.2  ");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (button == clockIn) {
			try {
				...
				JOptionPane.showMessageDialog(null, ex.getMessage(), "ClassNotFoundException",
						JOptionPane.ERROR_MESSAGE);
				Runnable.myLog.logger.warning(ex + " - " + Runnable.myLog.stackTraceToString(ex));
	...
	public int buttonDisabled() {
		...
		try {
			...
			Runnable.myLog.logger.warning(e + " - " + Runnable.myLog.stackTraceToString(e));
		...
		try {
			...
			Runnable.myLog.logger.warning(e1 + " - " + Runnable.myLog.stackTraceToString(e1));
		}
	...
	public long[] cuota(String date, boolean unfinishedTime) {
		...
		try {
			...
			boolean rs1 = rs[0].next();
			boolean rs2 = rs[1].next();

			while (rs1 && rs2) {

				SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
				date1 = sdf.parse(rs[0].getString(1) + " " + rs[0].getString(2));
				date2 = sdf.parse(rs[1].getString(1) + " " + rs[1].getString(2));
